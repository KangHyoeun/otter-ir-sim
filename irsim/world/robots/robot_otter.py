...
class RobotOtter(ObjectBase):
    ...
    def get_vel_range(self):
        """
        Override parent's get_vel_range() to disable acceleration limits.
        
        Otter USV uses full 6-DOF dynamics with physical constraints (propeller saturation,
        mass, damping, etc.). IR-SIM's acceleration limits would conflict with these physics-based
        constraints and cause numerical instability.
        
        Returns:
            tuple: (very_negative_limits, very_positive_limits) to effectively disable clipping
        """
        # Return very wide limits to disable velocity clipping
        # Otter dynamics handles all physical constraints internally
        import numpy as np
        
        very_negative = np.array([[-1000.0], [-1000.0]])
        very_positive = np.array([[1000.0], [1000.0]])
        
        return very_negative, very_positive
